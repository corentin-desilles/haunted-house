...
for (let i = 0; i < 50; i++) {
  ...
  const radius = 3 + Math.random() * 6 // Random radius from 3 to 9
  const x = Math.sin(angle) * radius // Get the x position using cos
  const z = Math.cos(angle) * radius // Gt the z position using sin
